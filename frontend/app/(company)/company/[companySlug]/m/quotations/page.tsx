        ...
        <QuotationDetail
          ...
          onPdf={async () => {
            ...
            if (detailQuotation.status === 'draft') {
              ...
              const updated = await companyApi.getQuotation(detailQuotation.id);
              setDetailQuotation(updated);
            }
          }}
          onStatusChange={async (id, status) => {
            await handleStatusUpdate(id, status);
            const updated = await companyApi.getQuotation(id);
            setDetailQuotation(updated);
          }}
        />
      )}
        ...
        <ShareLinkModal
          ...
          onLinkCreated={async () => {
            ...
            if (detailQuotation && detailQuotation.id === shareTarget.id) {
              const updated = await companyApi.getQuotation(shareTarget.id);
              setDetailQuotation(updated);
            }
          }}
        />
