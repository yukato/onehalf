...
export default function TicketsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.getTickets(searchQuery || undefined, limit, offset);
  ...
  return (
    <PageLayout currentPage="dashboard" title="Zendeskチケット一覧" onLogout={handleLogout}>
      <div className="mb-4 flex items-center gap-4">
        <SearchBox
          value={searchInput}
          onChange={setSearchInput}
          onSubmit={handleSearch}
          placeholder="件名またはチケットIDで検索..."
      ...
      <div className="flex-1 overflow-auto bg-white rounded-lg border border-gray-200">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 sticky top-0">
            <tr>
              <th className="text-left px-3 py-2 font-medium text-gray-600 w-24">チケットID</th>
              <th className="text-left px-3 py-2 font-medium text-gray-600">件名</th>
              <th className="text-left px-3 py-2 font-medium text-gray-600 w-24">ステータス</th>
