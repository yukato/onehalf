...
export default function MacrosPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await api.getMacros(searchQuery || undefined, limit, offset);
  ...
  return (
    <PageLayout currentPage="dashboard" title="Zendeskマクロ一覧" onLogout={handleLogout}>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 sticky top-0">
            <tr>
              ...
              <th className="text-center px-3 py-2 font-medium text-gray-600 w-20">FAQ</th>
              <th className="text-center px-3 py-2 font-medium text-gray-600 w-20">内部</th>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-center">
                  ...
                  >
                    {item.is_faq_enabled ? '可' : '-'}
                  </span>
                </td>
                <td className="px-3 py-2 text-center">
                  <span
                    className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                      item.is_internal_enabled
                        ? 'bg-blue-100 text-blue-700'
                        : 'bg-gray-100 text-gray-400'
                    }`}
                  >
                    {item.is_internal_enabled ? '可' : '-'}
                  </span>
                </td>
